...
int Xen_Map_Push_Pair(Xen_Instance* map_inst, Xen_Map_Pair pair) {
  ...
  while (current) {
    ...
    if (eval == Xen_True) {
      Xen_GC_Write_Field(&current->value, (Xen_GCHeader*)pair.value);
      map->__size++;
      return 1;
    }
  ...
  new_node->key = Xen_GCHandle_New((Xen_GCHeader*)map);
  new_node->value = Xen_GCHandle_New((Xen_GCHeader*)map);
  Xen_GC_Write_Field(&new_node->key, (Xen_GCHeader*)pair.key);
  Xen_GC_Write_Field(&new_node->value, (Xen_GCHeader*)pair.value);
