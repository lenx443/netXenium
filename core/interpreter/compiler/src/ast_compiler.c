...
static int compile_string(int out_reg, block_list_ptr blocks,
                          block_node_ptr block, Xen_Instance* ast) {
...
static int compile_literal(int out_reg, block_list_ptr blocks,
                           block_node_ptr block, Xen_Instance* ast) {
  int literal_index = Xen_SIZE(blocks->consts->c_names);
  if (!vm_consts_push_name(blocks->consts, Xen_AST_Node_Value(ast)))
    return 0;
  return ir_add_load_instance(block->instr_array, out_reg, literal_index);
}

static int compile_property(int out_reg, block_list_ptr blocks,
                            block_node_ptr block, Xen_Instance* ast) {
...
static int compile_cmd(block_list_ptr blocks, block_node_ptr block,
                       Xen_Instance* ast) {
  if (!block || !ast)
    return 0;
  ...
  if (!vm_consts_push_name(blocks->consts, Xen_AST_Node_Value(ast)))
    return 0;
  if (!ir_add_load_name(block->instr_array, 0, cmd_name_index))
    return 0;
